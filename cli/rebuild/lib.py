...
def rebuild_lang(lib, verbose):
    global file_list
    lib_path = microscope.lib_script / "microscope" / lib
    file_list = list()
    ...
    with tempfile.NamedTemporaryFile(suffix='.gdl', mode="w+") as output_file:
        ...
        with tempfile.NamedTemporaryFile(suffix='.gdl') as tmp_out:
            cmd += [str(microscope.datalog_script), str(output_file.name), "-p", str(microscope.lib_1_0), "-o", tmp_out.name]
            if verbose:
                cmd += ["--verbose"]
            tmp = Runner(cmd, 3600)
            ...
            if return_code != 0:
                ...
                return -1
            shutil.copy2(tmp_out.name, microscope.lib_1_0 / ("microscope." + lib + ".gdl"))
