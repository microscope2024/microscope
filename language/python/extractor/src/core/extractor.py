...
class Extractor(ast.NodeVisitor):
    ...
    def get_parent(self, node):
        ...
        if parent_dict.get(s):
            ...
            microscope_parent = Parent(
                parent_oid=s, parent_type=type(node).__name__)
            parent_dict[s] = microscope_parent
            parent = {
                "parent_oid": microscope_parent.parent_oid,
                "parent_type": microscope_parent.parent_type,
            }
            self.storage.store_parent(parent)
            return microscope_parent
    ...
    def calculate_location(self, node, oid):
        ...
        Args:
            ...
            oid: the microscope oid for the node
        Returns: None
        """
        location = {
            "element_oid": oid,
